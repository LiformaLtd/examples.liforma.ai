...
Hosted demos need `@liforma/client` **≥ 0.4.11** so `ExperienceWidget` expands preview catalog paths (`/avatars/…`, `/locations/…/scene`) onto `cdn.liforma.ai`. `0.4.2`–`0.4.10` leave those paths relative, so the FAB stays on the dark placeholder. `0.4.1` rejects nested `capabilities` from current `api.liforma.ai`. `0.4.0` still called removed `/v1/public-sessions` and failed CORS preflight.
