...
function apiBaseUrl(): string {
	return (process.env.LIFORMA_API_URL ?? 'https://api.liforma.ai').replace(/\/$/, '');
}

function projectId(): string {
	return process.env.LIFORMA_PROJECT_ID?.trim() || DEFAULT_PROJECT_ID;
}

function apiKey(): string | null {
	return process.env.LIFORMA_API_KEY?.trim() ?? null;
}
