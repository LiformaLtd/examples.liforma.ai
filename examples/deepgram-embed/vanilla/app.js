/**
 * Demo page shell for the Deepgram Voice Agent embed example.
...
import { EXPERIENCE_ID, SUGGESTED_AGENT } from './config.js';
import {
	DemoProxyReadyError,
...
const formEl = document.getElementById('deepgram-form');
...
function updateApiKeyHint() {
	...
	if (key.length < DEEPGRAM_API_KEY_MIN_LENGTH) {
		...
		apiKeyHintEl.textContent = `Key looks too short (${key.length} chars). Paste the full secret from the Deepgram console.`;
		return;
	}
	apiKeyHintEl.hidden = true;
	apiKeyHintEl.textContent = '';
...
function handleConnectError(err) {
	...
	log(message, 'warn', { href: DEEPGRAM_CONSOLE_URL, label: 'Open Deepgram console' });
	if (err instanceof DemoProxyReadyError && err.status === 400) {
		log('Provide a Deepgram API key in the form, or set DEEPGRAM_API_KEY on the server.', 'warn');
	}
}
...
async function armDeepgram() {
	...
	try {
		cachedProxyUrl = null;
		log('Validating Deepgram API key via local proxy…');
		const ready = await fetchDemoProxyReady(key);
		cachedProxyUrl = buildDeepgramProxyUrl({
			apiKey: key,
			useEnvKey: ready.useEnvKey,
			proxyPath: ready.proxyPath
		});
...
async function endDeepgram() {
	...
	if (active) {
		...
		log('Disconnected (cleared armed Deepgram connection).');
	}
	refreshStatus();
}
...
async function initExperience() {
	...
	experience = await Experience.startSession({
		...
		onStart: () => {
			...
			if (!armed) {
				...
				log('Start before Connect — connect to Deepgram below to begin the conversation.');
			}
		}
	});
	...
	const handleReady = () => {
		...
		refreshStatus();
		log('Experience ready. Connect Deepgram, then tap Start experience on the avatar.');
	};
