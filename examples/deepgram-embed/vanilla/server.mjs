#!/usr/bin/env node
/**
 * Static server for the Deepgram Voice Agent embed example + WebSocket proxy.
 * Usage: PORT=4008 node server.mjs
 *        (or from repo root via ./start)
 *
 * WS  /api/deepgram-agent[?apiKey=…]  → Deepgram agent.converse (Authorization on server)
 * POST /api/deepgram-proxy-ready       → validate key (form or DEEPGRAM_API_KEY)
 */
...
server.listen(port, host, () => {
	...
	console.log(`Deepgram Voice Agent embed → http://${host}:${port}/ (${label})`);
