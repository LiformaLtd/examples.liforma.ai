<script lang="ts">
	...
	let bridge = $state<DeepgramAgentBridge | null>(null);
	...
	function updateApiKeyHint(): void {
		...
		if (key.length < DEEPGRAM_API_KEY_MIN_LENGTH) {
			apiKeyHint = `Key looks too short (${key.length} chars). Paste the full secret from the Deepgram console.`;
			return;
		}
		apiKeyHint = '';
	}
	...
	async function armDeepgram(): Promise<void> {
	...
	async function endDeepgram(): Promise<void> {
		const active = bridge;
		bridge = null;
		connecting = false;
...
<div class="layout">
	<section class="workspace" aria-label="Deepgram Voice Agent embed">
		...
		<div class="control-card">
			...
			<p>
				...
				Deepgram with your key (demo-only — use a real backend in production).
			</p>
			...
			>
				<label class="field">
					...
					/>
					<span class="field-note">Never stored in this browser. Prefer env DEEPGRAM_API_KEY locally.</span>
				</label>
				...
				<div class="actions">
					...
					<button type="button" class="btn" disabled={endDisabled} onclick={() => void endDeepgram()}>
						End
					</button>
				</div>
			</form>
			...
			<p class="hint">
				Browsers cannot set <code>Authorization</code> on WebSocket — this demo uses a same-origin WS
				proxy (Vite plugin). Agent defaults live in <code>src/lib/config.ts</code>
				(<code>SUGGESTED_AGENT</code>). See the
				<a
