#!/usr/bin/env node
/**
 * Static server for the LiveKit embed example + participant-token mint.
 * Usage: PORT=4009 node server.mjs
 *
 * POST /api/livekit-token  { roomName?, identity? }
 * → { url, token } using LIVEKIT_URL / LIVEKIT_API_KEY / LIVEKIT_API_SECRET
 */
import { createServer } from 'node:http';
import { readFile, stat } from 'node:fs/promises';
import { join, extname, resolve, dirname, sep } from 'node:path';
import { fileURLToPath } from 'node:url';
import { handleLiveKitToken } from '../shared/mint-livekit-token.mjs';
...
const server = createServer(async (req, res) => {
	try {
		if (await handleLiveKitToken(req, res)) return;
...
server.listen(port, host, () => {
	...
	console.log(`LiveKit embed → http://${host}:${port}/ (${label})`);
	console.log(`Token mint: POST /api/livekit-token`);
});
