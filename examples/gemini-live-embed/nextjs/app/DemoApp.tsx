...
export default function DemoApp() {
	...
	const [bridge, setBridge] = useState<GeminiLiveBridge | null>(null);
	...
	const updateApiKeyHint = useCallback((key: string) => {
		...
		if (key.length < GEMINI_API_KEY_MIN_LENGTH) {
			setApiKeyHint(
				`Key looks too short (${key.length} chars). Paste the full secret from the Gemini console.`
			);
			return;
		}
		setApiKeyHint('');
	...
	const armGemini = useCallback(async () => {
		...
		try {
			...
			const ready = await fetchDemoProxyReady(key);
			const proxyUrl = buildGeminiProxyUrl({
				apiKey: key,
				useEnvKey: ready.useEnvKey,
				proxyPath: ready.proxyPath
			});
	...
	const endGemini = useCallback(async () => {
		const active = bridge;
		setBridge(null);
		setConnecting(false);
	...
	return (
		<>
			<div className="demo-layout">
				<section className="demo-workspace" aria-label="Gemini Live embed">
					...
					<div className="demo-control-card">
						...
						<p>
							...
							forwards to Gemini with your key (demo-only — use a real backend in production).
						</p>
						...
						>
							<label className="demo-field">
								...
								<span className="demo-field-note">
									Never stored in this browser. Prefer env GEMINI_API_KEY or GOOGLE_API_KEY locally.
								</span>
							</label>
						...
						<p className="demo-hint">
							API keys stay on the server — this demo uses a
							same-origin WS proxy (custom server.mjs). Setup (model / system instruction) is injected by the proxy; see{' '}
							<code>lib/config.ts</code> (<code>GEMINI_LIVE_*</code> env). See the{' '}
							<a
