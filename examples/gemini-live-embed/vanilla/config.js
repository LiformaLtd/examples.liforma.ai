/**
 * Demo content only — not part of the Liforma ↔ Gemini Live integration.
 * Integration: copy helloByo.js → startByoSpeech → connectGeminiLive
 *
 * System instruction + model setup are injected by the local WS proxy
 * (`shared/gemini-live-proxy.mjs`). Override with GEMINI_LIVE_SYSTEM_INSTRUCTION /
 * GEMINI_LIVE_MODEL on the server.
 */
...
/**
 * Display-only copy of the barista system instruction applied by the proxy setup.
 * The connectGeminiLive helper does not send setup — the proxy does.
 */
export const SUGGESTED_SYSTEM_INSTRUCTION = `You are Anna, a friendly barista at London Coffee, a cosy café in London. You speak through an animated avatar standing behind the service counter.
