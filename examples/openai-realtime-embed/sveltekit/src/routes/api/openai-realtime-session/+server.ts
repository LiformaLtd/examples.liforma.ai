...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (!apiKey) {
		apiKey = normalizeApiKey(String(env.OPENAI_API_KEY ?? ''));
	}
	...
	const model = String(env.OPENAI_REALTIME_MODEL ?? DEFAULT_REALTIME_MODEL).trim();
	const voice = String(env.OPENAI_REALTIME_VOICE ?? DEFAULT_REALTIME_VOICE).trim();
	...
	try {
		...
		return json({ error: 'Unexpected OpenAI response' }, { status: 502 });
	}

	if (!value) {
		return json({ error: 'OpenAI response missing client secret value' }, { status: 502 });
	}

	return json({ value, model, voice });
};
